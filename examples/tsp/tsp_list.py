...
class TSP(MeasurementInterface):
    def __init__(self, args):
        ...
        self.lookup = [list(e) for e in permutations(self.elements)]
        assert len(self.lookup) == factorial(len(self.elements))

    def run(self, desired_result, input, limit):
        cfg = desired_result.configuration.data
        p = self.lookup[cfg[0]]  # cheating: should use manipulator function
        t = self.eval_path(p)
